...
import { existsDirectory, mkdir, readdir } from './exports/FileSystem.js';
...
export class PluginManager {
	...
	public async initialize() {
		...
		const files = (await readdir(['plugins'])).filter((e) =>
			e.endsWith('.js'),
		);

		for (const file of files) {
			try {
				const pluginFile = await import(
					`#plugins/${file.split('.')[0]}`
				);
		...
		for (const plugin of this.getPlugins()) {
			...
			try {
				...
				this.client.logger
					.error(
						`[PluginManager] Error loading ${plugin.name} plugin.`,
					)
					.error(err);
		...
		this.client.logger.info(
			`[PluginManager] - ${
				this.client.dispatcher.inhibitors.length
			} Inhibitor${
